...
from luisa.autodiff import requires_grad, autodiff, backward, grad, detach
...
@luisa.func
def direct_estimator(ray, sampler, heap, accel, light_count, env_count, material_buffer, texture_res):
    hit = accel.trace_closest(ray, -1)
    if hit.miss():
        return heap.texture2d_sample(23332, direction_to_uv(ray.get_dir())).xyz if env_count > 0 else float3(0.0)
    ...
    # direct light sample
    radiance = float3(0.0)
    light = sample_light(it.p, light_count, env_count, heap, accel, sampler) # (wi, dist, pdf, eval)
...
@luisa.func
def direct_estimator_backward(ray, sampler, heap, accel, light_count, env_count,
    ...
    if not occluded and wi_light_local.z > 0.0:
        ...
        with autodiff():
            requires_grad(mat)
            bsdf = ggx_brdf(wo_local, wi_light_local, mat.xyz, 0.04, mat.w)
