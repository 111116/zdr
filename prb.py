    ...
    for depth in range(max_depth):
        ...
        if not occluded and wi_light_local.z >= 1e-4:
            with autodiff():
                ...
                pdf_bsdf = ggx_sample_pdf(wo_local, wi_light_local, diffuse, specular, roughness)
                mis_weight = balanced_heuristic(light.pdf, pdf_bsdf)
                backward(bsdf, beta * mis_weight * light.eval / max(light.pdf, 1e-4) * le_grad)
                mat_grad += grad(mat)
            Le -= beta * bsdf * mis_weight * light.eval / max(light.pdf, 1e-4)
        ...
        with autodiff():
            ...
            diffuse = mat.xyz
            roughness = mat.w
            bsdf = ggx_brdf(wo_local, wi_local, diffuse, specular, roughness)
            backward(bsdf, beta / pdf_bsdf * Le * le_grad)
            mat_grad += grad(mat)
        beta *= bsdf / pdf_bsdf
        ...
        # write gradient
        if any(mat_grad != float4(0.0)):
            if not any(isnan(mat_grad)):
                write_bsdf_grad(it.uv, mat_grad, d_material_buffer, texture_res)
            mat_grad = float4(0.0)
    
    # write gradient after loop
    if any(mat_grad != float4(0.0)):
        if not any(isnan(mat_grad)):
            write_bsdf_grad(it.uv, mat_grad, d_material_buffer, texture_res)
        mat_grad = float4(0.0)
